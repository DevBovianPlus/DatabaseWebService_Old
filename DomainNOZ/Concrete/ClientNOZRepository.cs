...
{
    public class ClientNOZRepository : IClientNOZRepository
    {
        GrafolitNOZEntities context;

        public ClientNOZRepository(GrafolitNOZEntities _context)
        {
            context = _context;
        }
            ...
            {//loj => left outer join
                var query = from client in context.Stranka_NOZ
                            select new ClientSimpleModel
                            {
                                ...
                                TipStranka = client.TipStranka_NOZ != null ? client.TipStranka_NOZ.Naziv : "",
        ...
        public bool DeleteClient(int clientID)
        {
            try
            {
                var client = context.Stranka_NOZ.Where(s => s.StrankaID == clientID).FirstOrDefault();

                if (client != null)
                {
                    context.Stranka_NOZ.Remove(client);
                    context.SaveChanges();
                    return true;
                }
        ...
        public List<ClientEmployeeModel> GetClientEmployeeModelList(int clientID)
        {
            try
            {
                var query = from cE in context.StrankaZaposleni_NOZ
        ...
        {
            StrankaZaposleni_NOZ strZap = new StrankaZaposleni_NOZ();
            ...
            try
            {
                ...
                if (strZap.StrankaOsebeID == 0)
                {
                    ...
                    context.StrankaZaposleni_NOZ.Add(strZap);
                    context.SaveChanges();
                }
                else
                {
                    if (updateRecord)
                    {
                        StrankaZaposleni_NOZ original = context.StrankaZaposleni_NOZ.Where(sz => sz.StrankaOsebeID == strZap.StrankaOsebeID).FirstOrDefault();
        ...
        public bool DeleteClientEmployee(int clientID, int employeeID)
        {
            try
            {
                var clientEmployee = context.StrankaZaposleni_NOZ.Where(sz => sz.StrankaID == clientID && sz.OsebeID == employeeID).FirstOrDefault();

                if (clientEmployee != null)
                {
                    context.StrankaZaposleni_NOZ.Remove(clientEmployee);
                    context.SaveChanges();
                    return true;
                }
       ...
       /*public List<ContactPersonModel> GetContactPersonModelList(int clientID)
        ...
        public ClientType GetClientTypeByCode(string typeCode)
        {
            try
            {
                var query = from type in context.TipStranka_NOZ
