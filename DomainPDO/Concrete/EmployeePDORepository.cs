...
{
    public class EmployeePDORepository : IEmployeePDORepository
    {
        GrafolitPDOEntities context;

        public EmployeePDORepository(GrafolitPDOEntities _context)
        {
            context = _context;
        }
        ...
        public List<EmployeeFullModel> GetAllEmployees()
        {
            try
            {
                var query = from emp in context.Osebe_PDO
                            ...
                            select new EmployeeFullModel
                            {
                                ...
                                PDODostop = employee.Key.PDODostop.HasValue ? employee.Key.PDODostop.Value : false,
        ...
        public EmployeeFullModel GetEmployeeByID(int employeeID)
        {
            try
            {
                var query = from employee in context.Osebe_PDO
                            ...
                            select new EmployeeFullModel
                            {
                                ...
                                Vloga = (from role in context.Vloga_PDO
                                ...
                                NadrejeniID = context.OsebeNadrejeni_PDO.Where(osn => osn.OsebaID == employeeID).FirstOrDefault() != null ? context.OsebeNadrejeni_PDO.Where(osn => osn.OsebaID == employeeID).FirstOrDefault().OsebeNadrejeniID : 0,
        ...
        public bool DeleteEmployee(int employeeID)
        {
            try
            {
                var employee = context.Osebe_PDO.Where(e => e.OsebaID == employeeID).FirstOrDefault();
                if (employee != null)
                {
                    context.Osebe_PDO.Remove(employee);
                    context.SaveChanges();
                    return true;
                }
        ...
        public List<RoleModel> GetRoles()
        {
            try
            {
                var query = from role in context.Vloga_PDO
                            select new RoleModel
                            {
                                idVloga = role.VlogaID,
                                Koda = role.Koda,
